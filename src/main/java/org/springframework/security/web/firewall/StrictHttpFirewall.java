...
/**
 *HttpFirewall的严格实现，它拒绝任何带有RequestRejectedException的可疑请求。
 * 以下规则适用于防火墙：
 * 拒绝不允许的 HTTP 方法。这指定阻止HTTP 动词篡改和 XST 攻击。参见setAllowedHttpMethods(Collection)
 * 拒绝未规范化的 URL 以避免绕过安全约束。无法禁用此约束，因为禁用此约束被认为风险极大。允许此行为的一些选项是在防火墙之前规范化请求或使用DefaultHttpFirewall代替。请记住，规范化请求是脆弱的，以及为什么请求被拒绝而不是规范化。
 * 拒绝包含不可打印的 ASCII 字符的 URL。无法禁用此约束，因为禁用此约束被认为风险极大。
 * 拒绝包含分号的 URL。请参见setAllowSemicolon(boolean)
 * 拒绝包含 URL 编码斜杠的 URL。请参见setAllowUrlEncodedSlash(boolean)
 * 拒绝包含反斜杠的 URL。参见setAllowBackSlash(boolean)
 * 拒绝包含 URL 编码百分比的 URL。参见setAllowUrlEncodedPercent(boolean)
 * 拒绝不允许的主机。请参阅setAllowedHostnames(Predicate)
 * @author Rob Winch
